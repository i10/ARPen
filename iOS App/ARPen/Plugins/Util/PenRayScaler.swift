...
class PenRayScaler {
    ...
    func update(scene: PenScene, buttons: [Button : Bool]) {
        ...
        if selectedTargets.count == 1
        {
            ...
            {
                let hit = hitTest(pointerPosition: scene.pencilPoint.position)
                
                if hit == selectedTargets.first
                {
                    dragging = true
                        
                    let projPosGeo = projectOntoImagePlane(pointerPosition: selectedTargets.first!.position)
    ...
    func viewBoundingBox(_ target: ARPGeomNode) {
        ...
        let projRFU = projectOntoImagePlane(pointerPosition: rfu)
        let projPos = projectOntoImagePlane(pointerPosition: target.position)
        
        let halfHeight = abs(Float(projRFU!.y - projPos!.y))
        //first time we selected geometry so store new value
        if !(originalHalfHeight.keys.contains(ref!)){
