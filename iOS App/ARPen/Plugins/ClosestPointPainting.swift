...
class ClosestPointPainting: Plugin, UserStudyRecordPluginProtocol {
    
    var recordManager: UserStudyRecordManager!
    
    
    var pluginImage : UIImage? = UIImage.init(named: "ClosestPointPainting")
    var pluginInstructionsImage: UIImage? = UIImage.init(named: "ClosestPointPaintingPluginInstructions")
    var pluginIdentifier: String = "Closest Point"
    ...
    func didUpdateFrame(scene: PenScene, buttons: [Button : Bool]) {
        ...
        //let objectList = scene.rootNode.childNodes
        //let objectList = scene.drawingNode.childNodes
        ...
        if (objectList.contains(where: {($0.name?.hasPrefix("currentStudyObject") ?? false)})){
            
            for object in objectList{
                if (object.name?.hasPrefix("currentStudyObjectNode") ?? false){
                    for obj in object.childNodes{
                        ...
                        if (obj.name?.hasPrefix("currentStudyObjectBoxNode") ?? false){
                            ...
                            var localPosition = scene.pencilPoint.convertPosition(SCNVector3Zero, to: obj)
                            
                            // clamp coordinates to box size
                            
                            if (localPosition.x > Float((obj.geometry as! SCNBox).width)/2){
                                localPosition.x = Float((obj.geometry as! SCNBox).width)/2
                            }
                            if (localPosition.x < Float(-(obj.geometry as! SCNBox).width)/2){
                                localPosition.x = Float(-(obj.geometry as! SCNBox).width)/2
                            }
                            
                            if (localPosition.y > Float((obj.geometry as! SCNBox).height)/2){
                                localPosition.y = Float((obj.geometry as! SCNBox).height)/2
                            }
                            if (localPosition.y < Float(-(obj.geometry as! SCNBox).height)/2){
                                localPosition.y = Float(-(obj.geometry as! SCNBox).height)/2
                            }
                            
                            if (localPosition.z > Float((obj.geometry as! SCNBox).length)/2){
                                localPosition.z = Float((obj.geometry as! SCNBox).length)/2
                            }
                            if (localPosition.z < Float(-(obj.geometry as! SCNBox).length)/2){
                                localPosition.z = Float(-(obj.geometry as! SCNBox).length)/2
                            }
                            ...
                            if (localPosition.x > Float(-(obj.geometry as! SCNBox).width)/2 &&
                                ...
                                if (localPosition.x >= 0){
                                    deltaX = Float((obj.geometry as! SCNBox).width)/2 - localPosition.x
                                }
                                if (localPosition.x < 0){
                                    deltaX = -Float((obj.geometry as! SCNBox).width)/2 - localPosition.x
                                }
                                if (localPosition.y >= 0){
                                    deltaY = Float((obj.geometry as! SCNBox).height)/2 - localPosition.y
                                }
                                if (localPosition.y < 0){
                                    deltaY = -Float((obj.geometry as! SCNBox).height)/2 - localPosition.y
                                }
                                if (localPosition.z >= 0){
                                    deltaZ = Float((obj.geometry as! SCNBox).length)/2 - localPosition.z
                                }
                                if (localPosition.z < 0){
                                    deltaZ = -Float((obj.geometry as! SCNBox).length)/2 - localPosition.z
                                }
                            ...
                            surfacePosition = obj.convertPosition(localPosition, to: nil)
                            distance = surfacePosition.distance(vector: scene.pencilPoint.position)
                        }
                        if (obj.name?.hasPrefix("currentStudyObjectSphereNode") ?? false){
                            // use positioning with sphere
                            
                            // distance = distance(sphere, point) - sphere.radius
                            
                            // surfacePosition = sphere_pos + (pen_pos-sphere_pos) * (radius/distance distance(sphere, point))
                            
                            distance = scene.pencilPoint.position.distance(vector: obj.position) - Float((obj.geometry as! SCNSphere).radius)
                            
                            // this is to prevent a division by zero, which could occur if pen is positioned exactly in sphere's center
                            if (scene.pencilPoint.position.distance(vector: obj.position) == 0){
                                surfacePosition = obj.position + (scene.pencilPoint.position - obj.position) * (Float((obj.geometry as! SCNSphere).radius)/(scene.pencilPoint.position.distance(vector: obj.position) + 0.0001))
                            }
                            else {
                                surfacePosition = obj.position + (scene.pencilPoint.position - obj.position) * (Float((obj.geometry as! SCNSphere).radius)/scene.pencilPoint.position.distance(vector: obj.position))
                            }
                            
                        }
                        if (obj.name?.hasPrefix("GuideCylinder") ?? false){
                            // use positioning with cylinder
                            
                            // convert pen position to cylinder's local space
                            
                            var localPosition = scene.pencilPoint.convertPosition(SCNVector3Zero, to: obj)
                            // if height over or under cylinder:
                            // clamp height and skip inside volume checking
                            if (localPosition.y > Float((obj.geometry as! SCNCylinder).height)){
                                localPosition.y = Float((obj.geometry as! SCNCylinder).height)
                            }
                            if (localPosition.y < -Float((obj.geometry as! SCNCylinder).height)){
                                localPosition.y = -Float((obj.geometry as! SCNCylinder).height)
                            }
                            
                            var flatPenPosition = SCNVector3(localPosition.x, 0, localPosition.z)
                            
                            // this is to prevent a division by zero, which could occur if pen is positioned exactly on cylinder axis
                            if (flatPenPosition.x == 0 && flatPenPosition.z == 0){
                                flatPenPosition.x = 0.01
                            }
                            surfacePosition = flatPenPosition * (Float((obj.geometry as! SCNCylinder).radius)/flatPenPosition.distance(vector: SCNVector3Zero))
                            
                            surfacePosition.y = localPosition.y
                            
                            surfacePosition = obj.convertPosition(surfacePosition, to: nil)
                            
                            distance = scene.pencilPoint.position.distance(vector: surfacePosition)
                            
                        }
                        if (distance > 0 && distance < closestDistance || closestDistance < 0){
                            closestDistance = distance
                            closestPosition = surfacePosition
                        }
                    }
                }
            }
        ...
        // if no valid position on object was found, default to pencil point
        if closestPosition == nil {
            closestPosition = scene.pencilPoint.position
        }
        ...
        if pressed, let previousPoint = self.previousPoint {
            ...
            scene.directionNode.position = closestPosition!
            ...
            if (objectList.contains(where: {($0.name?.hasPrefix("currentStudyObjectNode") ?? false)})){
                for obj in objectList{
                    var distance = Float(-1)
                    var surfacePosition = SCNVector3Zero
                    //if (obj.name?.hasPrefix("GuideBox") ?? false){
            ...
            "CurrentError" : String(describing: closestDistance),
            "AverageError" : String(describing: averageError),
        ...
        self.previousPoint = closestPosition
        scene.projectionNode.position = closestPosition!
