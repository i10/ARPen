...
class PluginManager: ARManagerDelegate, PenManagerDelegate {
    ...
    init(penScene: PenScene, sceneView: ARSCNView) {
        ...
        self.plugins = [paintPlugin, DirectPenScalingPlugin(), PenRayScalingPlugin(), PinchScalingPlugin(), ScrollScalingPlugin(),  TouchAndPenScalingPlugin(), PointScalingPlugin(), CubeByDraggingPlugin(), SphereByDraggingPlugin(), CylinderByDraggingPlugin(), PyramidByDraggingPlugin(), CubeByExtractionPlugin(), TranslationDemoPlugin(), CombinationPlugin(), RotationWithPenPlugin(), RotationWithDevicePlugin(), RotationWithTSPlugin(), RotationWithPenPedalPlugin(), RotationWithDevicePedalPlugin(), MidairPlugin(), LinearMenuPlugin(), TwoHandedTouch(), HapticMenuPlugin(), MovingCursorPlugin(), ModelingPlugin(), SweepPluginProfileAndPath(), SweepPluginTwoProfiles(), LoftPlugin(), RevolvePluginProfileAndAxis(), RevolvePluginProfileAndCircle(), RevolvePluginTwoProfiles(), CombinePluginFunction(), CombinePluginSolidHole(), DepthRayPlugin(), BubblePlugin(), ShadowPlugin(), HeatmapPlugin()]
